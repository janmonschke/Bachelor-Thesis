...
var Slideshow = (function(){
	...
	function updatePresenterWindow(){
	  if(presenter_window != null && presenter_window.window != undefined){
		  var new_url = window.location.hash.split("/");
		  var _new = new_url[new_url.length - 1];
		  var p_new_url = presenter_window.window.location.hash.split("/");
		  p_new_url[p_new_url.length - 1] = _new++;
		  p_new_url = p_new_url.join("/");
		  //alert(p_new_url);
		  loc = presenter_window.window.location;
		  presenter_window.window.location.href = loc.protocol + "//" + loc.pathname + p_new_url;
		}
	};
